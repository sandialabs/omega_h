template <Int dim>
OSH_INLINE Real metric_product(
    Matrix<dim,dim> m,
    Vector<dim> v) {
  return v * (m * v);
}
...
template <Int dim>
OSH_INLINE Real metric_desired_length(
    Matrix<dim,dim> m,
    Vector<dim> dir) {
...
template <Int dim>
OSH_INLINE Vector<dim> metric_lengths(Vector<dim> l) {
  Vector<dim> h;
  for (Int i = 0; i < dim; ++i)
    h[i] = 1.0 / sqrt(l[i]);
  return h;
...
template <Int dim>
OSH_INLINE Matrix<dim,dim> compose_metric(
    Matrix<dim,dim> r,
    Vector<dim> h) {
...
template <Int dim>
OSH_INLINE void decompose_metric(
    Matrix<dim,dim> m,
    Matrix<dim,dim>& r,
    Vector<dim>& h) {
  Vector<dim> l;
...
template <Int dim>
OSH_INLINE Matrix<dim,dim> common_metric_basis(
    Matrix<dim,dim> a,
    Matrix<dim,dim> b) {
  auto c = invert(a) * b;
  Matrix<dim,dim> p;
...
template <Int dim>
OSH_INLINE Matrix<dim,dim> intersect_metrics(
    Matrix<dim,dim> a,
    Matrix<dim,dim> b) {
...
OSH_INLINE Matrix<dim,dim> average_metrics(
