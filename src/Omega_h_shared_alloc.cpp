...
void stop_tracking_allocations(Library* lib) {
  OMEGA_H_CHECK(global_allocs != nullptr);
  auto comm = lib->world();
  auto mpi_high_water_bytes = comm->allreduce(
      I64(global_allocs->high_water_bytes), OMEGA_H_MAX);
  auto candidate_rank =
    (I64(global_allocs->high_water_bytes) == mpi_high_water_bytes) ?
    comm->rank() : comm->size();
  auto winner_rank = comm->allreduce(
      candidate_rank, OMEGA_H_MIN);
  if (comm->rank() == winner_rank) {
    std::stringstream ss;
    ss << "MPI rank " << comm->rank()
      << " had the highest memory high water mark.\n";
    ss << "The high water memory usage was "
      << global_allocs->high_water_bytes
...
Alloc::Alloc(
    std::size_t size_in, std::string const& name_in):
  size(size_in),
  name(name_in)
{
  init();
...
Alloc::Alloc(
    std::size_t size_in, std::string&& name_in):
  size(size_in),
  name(name_in)
{
  init();
...
void Alloc::init()
{
  ...
  if (ga) {
    ...
    if (ga->total_bytes > ga->high_water_bytes) {
      ...
      for (auto a = ga->first; a; a = a->next) {
        ga->high_water_records.push_back(
            {a->name, a->size});
      }
    }
  }
