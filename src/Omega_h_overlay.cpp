...
Overlay::Overlay(int* argc, char*** argv
   ...
   ,MPI_Comm comm
    ...
    mesh(build_box(library.world(), OMEGA_H_HYPERCUBE,
                   Nx * cellSize, Ny * cellSize, Nz * cellSize,
                   LO(Nx), LO(Ny), LO(Nz)))
{
...
const std::vector<size_t> & Overlay::get_cells() const {
  return silly_cells;
}

const std::vector<size_t> & Overlay::get_edges() const {
  return silly_edges;
}

const std::vector<size_t> & Overlay::get_faces() const {
  return silly_faces;
}

const std::vector<size_t> & Overlay::get_nodes() const {
  return silly_nodes;
}
...
size_t Overlay::get_invalid_cell_handle() const {
  return ~size_t(0);
}

double Overlay::get_cell_size() const {
  return cell_size;
}
