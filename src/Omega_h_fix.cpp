...
static void compute_ill_metric(Mesh* mesh, AdaptOpts const& opts) {
  if (mesh->dim() == 3) compute_ill_metric_dim<3>(mesh, opts);
  else if (mesh->dim() == 2) compute_ill_metric_dim<2>(mesh, opts);
  else if (mesh->dim() == 1) compute_ill_metric_dim<1>(mesh, opts);
