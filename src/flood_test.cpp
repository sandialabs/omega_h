...
int main(int argc, char** argv) {
  ...
  auto obj_vert_warps = repeat_vector(obj_verts.size(), vector_2(0, -3. / 8.));
  auto vert_warps =
      map_onto(obj_vert_warps, obj_verts, mesh.nverts(), 0.0, dim);
  mesh.add_tag(VERT, "warp", dim, vert_warps);
  auto densities = map_onto(Reals(obj_elems.size(), obj_density), obj_elems,
      mesh.nelems(), air_density, 1);
