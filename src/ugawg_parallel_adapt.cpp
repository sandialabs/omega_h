...
int main(int argc, char** argv) {
  ...
  auto final_nelems = Omega_h::get_expected_nelems_from_complexity(final_complexity, 3);
  auto nelems_per_rank = final_nelems / world->size();
  auto world_rank = world->rank();
  if (world_rank == 0) std::cout << "Estimating " << final_nelems << " final elements for the final complexity of " << final_complexity << '\n';
  auto self_comm = lib.self();
  Omega_h::Mesh mesh(&lib);
  if (world_rank == 0) {
    std::cout << "targeting " << final_nelems << " total elements, " << nelems_per_rank << " elements per rank\n";
  ...
  //Omega_h::vtk::write_vtu("input.vtu", &mesh);
    ...
    auto predicted_nelems = Omega_h::get_expected_nelems_from_complexity(complexity, mesh.dim());
  ...
  for (int subcomm_size = 2; subcomm_size <= world->size(); subcomm_size *= 2) {
    ...
    //Omega_h::vtk::write_parallel("ghosted", &mesh, Omega_h::vtk::dont_compress);
      ...
      if (world_rank == 0) std::cout << "Scaling the implied metric to get " << nsubcomm_elems << " elements in " << subcomm_size << " ranks\n";
      auto scalar = Omega_h::get_metric_scalar_for_nelems(mesh.dim(), nglobal_elems, nsubcomm_elems);
      if (world_rank == 0) std::cout << "Metric scalar was " << scalar << '\n';
      auto target_metric = Omega_h::multiply_each_by(implied_metric, scalar);
      mesh.add_tag(0, "target_metric", Omega_h::symm_ncomps(mesh.dim()), target_metric);
      if (world_rank == 0) std::cout << "Adapting the mesh to get " << nsubcomm_elems << " elements in " << subcomm_size << " ranks\n";
      auto adapt_opts = Omega_h::AdaptOpts(&mesh);
      adapt_opts.verbosity = Omega_h::EXTRA_STATS;
      while (Omega_h::approach_metric(&mesh, adapt_opts)) {
        Omega_h::adapt(&mesh, adapt_opts);
      ...
      if (world_rank == 0) std::cout << "Wound up with " << nglobal_elems << " elements\n";
    }
    world->barrier();
  }
  auto t2 = Omega_h::now();
  if (world_rank == 0) std::cout << "Tprep " << (t1 - t0) << " Tadapt " << (t2 - t1) << " Ttotal " << (t2 - t0) << '\n';
}
