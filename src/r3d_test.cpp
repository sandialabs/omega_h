...
static void test_3d() {
  ...
  OMEGA_H_CHECK(osh::are_close(faces[0].d, -faces[0].n[0]));
  for (osh::Int i = 0; i < 3; ++i) {
    auto v = osh::vector_3(0, 0, 0);
    v[i] = 1;
    OMEGA_H_CHECK(osh::are_close(faces[i + 1].n, v));
    OMEGA_H_CHECK(osh::are_close(faces[i + 1].d, 0));
  }
  ...
  OMEGA_H_CHECK(osh::are_close(volume, osh::cube(0.5) / 6.0));
  auto c =
      osh::r3d::clip(a, osh::Few<osh::r3d::Plane<3>, 1>({{{-1, 0, 0}, 0.5}}));
  OMEGA_H_CHECK(c.nverts == 6);
  volume = osh::r3d::measure(c);
  OMEGA_H_CHECK(osh::are_close(volume, (1. / 6.) - (osh::cube(0.5) / 6.)));
  osh::Few<osh::Vector<3>, 4> verts2 = {
      {1, 0, 0}, {1, 1, 0}, {0, 0, 0}, {1, 0, 1}};
  volume = osh::r3d::measure(osh::r3d::intersect_simplices(verts, verts2));
  OMEGA_H_CHECK(osh::are_close(volume, (1. / 3.) * (1. / 4.) * (1. / 2.)));
}

static void test_2d() {
  osh::Few<osh::Vector<2>, 3> verts = {{0, 0}, {1, 0}, {0, 1}};
  auto faces = osh::r3d::faces_from_verts(verts);
  OMEGA_H_CHECK(osh::are_close(faces[0].n, osh::vector_2(0, 1)));
  OMEGA_H_CHECK(osh::are_close(faces[0].d, 0));
  OMEGA_H_CHECK(osh::are_close(faces[1].n, -osh::normalize(osh::vector_2(1, 1))));
  OMEGA_H_CHECK(osh::are_close(faces[1].d, -faces[1].n[0]));
  OMEGA_H_CHECK(osh::are_close(faces[2].n, osh::vector_2(1, 0)));
  OMEGA_H_CHECK(osh::are_close(faces[2].d, 0));
  auto a = osh::r3d::init(verts);
