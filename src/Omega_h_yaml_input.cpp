  ...
  virtual void at_reduce(any& result_any, int prod, std::vector<any>& rhs) {
    switch (prod) {
      ...
      case yaml::PROD_DOC2: {
        ...
        OMEGA_H_CHECK(rhs.at(offset).type() == typeid(NamedInput));
        return std::move(rhs.at(offset));
      }
      case yaml::PROD_TOP_BMAP: {
        OMEGA_H_CHECK(!rhs.at(0).empty());
        OMEGA_H_CHECK(rhs.at(0).type() == typeid(NamedInput));
        return std::move(rhs.at(0));
      }
      case yaml::PROD_TOP_FIRST: {
        if (rhs.at(0).type() == typeid(MapInput)) {
          return std::move(rhs.at(0));
        }
        break;
      }
      ...
      case yaml::PROD_FMAP_FIRST: {
        ...
        OMEGA_H_CHECK(result_any.type() == typeid(MapInput));
        return result_any;
      }
      ...
      case yaml::PROD_FMAP_FSEQ: {
        ...
        return map_item(rhs.at(0), rhs.at(4), scalar_type);
      }
      case yaml::PROD_BMAP_BSCALAR: {
        return map_item(rhs.at(0), rhs.at(3), Scalar::STRING);
      }
      ...
      case yaml::PROD_BMAP_FSEQ: {
        OMEGA_H_CHECK(rhs.at(4).type() == typeid(ListInput));
        int scalar_type = interpret_tag(rhs.at(3));
        return map_item(rhs.at(0), rhs.at(4), scalar_type);
      }
