  ...
  if (y->M_color != S_rb_tree_red) {
    while (x != root && (x == nullptr || x->M_color == S_rb_tree_black))
      if (x == x_parent->M_left) {
        ...
        if ((w->M_left == nullptr ||
             w->M_left->M_color == S_rb_tree_black) &&
            (w->M_right == nullptr ||
             w->M_right->M_color == S_rb_tree_black)) {
...
protected:
  ...
  Rb_tree_node<Tp>* M_get_node()
    { return static_cast<Rb_tree_node<Tp>*>(std::malloc(sizeof(Rb_tree_node<Tp>))); }
...
public:
  ...
  Rb_tree()
    : Base(), M_node_count(0), M_key_compare()
    { M_empty_initialize(); }
...
template <class Key, class Value, class KeyOfValue, class Compare>
inline
std::pair<typename Rb_tree<Key, Value, KeyOfValue, Compare>::const_iterator,
     typename Rb_tree<Key, Value, KeyOfValue, Compare>::const_iterator>
Rb_tree<Key, Value, KeyOfValue, Compare>
...
struct Rb_first_as_key {
  T1 const& operator()(std::pair<T1, T2> const& value) const { return value.first; }
};
