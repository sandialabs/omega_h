...
int main(int argc, char** argv) {
  ...
  mesh.add_tag(
      osh::VERT, "normal", mesh.dim(), OSH_DONT_TRANSFER, vert_normals);
  ...
  if (mesh.dim() == 3) {
    ...
    mesh.add_tag(
        osh::EDGE, "tangent", mesh.dim(), OSH_DONT_TRANSFER, edge_tangents);
    ...
    mesh.add_tag(
        osh::VERT, "tangent", mesh.dim(), OSH_DONT_TRANSFER, vert_tangents);
