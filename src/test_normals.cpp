...
int main(int argc, char** argv) {
  ...
  auto surf_side_normals = Omega_h::surf::get_side_normals(&mesh, surf_side2side);
  ...
  if (mesh.dim() == 3) {
    auto edges_are_curv = Omega_h::mark_by_class_dim(&mesh, Omega_h::EDGE, Omega_h::EDGE);
    auto verts_are_curv = Omega_h::mark_by_class_dim(&mesh, Omega_h::VERT, Omega_h::EDGE);
    ...
    mesh.add_tag(
        Omega_h::EDGE, "tangent", mesh.dim(), OMEGA_H_DONT_TRANSFER, edge_tangents);
    ...
    mesh.add_tag(
        Omega_h::VERT, "tangent", mesh.dim(), OMEGA_H_DONT_TRANSFER, vert_tangents);
