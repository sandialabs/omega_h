...
void pybind11_c(py::module& module) {
  py::enum_<Omega_h_Source>(module, "Source", "The type of source of a metric field")
    ...
    .export_values();
  py::enum_<Omega_h_Isotropy>(module, "Isotropy", "Whether and how to convert an anisotropic metric into an isotropic one")
    .value("ANISOTROPIC", OMEGA_H_ANISOTROPIC)
    .value("ISO_LENGTH", OMEGA_H_ISO_LENGTH)
    .value("ISO_SIZE", OMEGA_H_ISO_SIZE)
    .export_values();
  py::enum_<Omega_h_Scales>(module, "Scales", "Whether a metric source scales to satisfy element counts")
