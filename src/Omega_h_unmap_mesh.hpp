...
/* Given an injective map (new_ents2old_ents) and its inverse (old_ents2new_ents),
   get the owners of (ent_dim) in the old mesh and unmap to what they will
   be in the new mesh.
   For each *owner*, new_index = old_ents2new_ents[old_index].
   These new indices are then communicated to the copies and returned
   in the order indicated by (new_ents2old_ents) */
