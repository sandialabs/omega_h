...
DEVICE Int find_distance_2_elems(Graph const& keys2elems,
    LOs const& elems2verts, Int nverts_per_elem, Graph const& verts2elems,
    Read<I8> const& indset, LO key, LO* stack, Int stack_max) {
...
DEVICE Int find_distance_3_keys(Graph const& keys2buf_elems,
    LOs const& elems2verts, Int nverts_per_elem, Graph const& verts2elems,
    LOs const& elems2keys, Int nkeys_per_elem, Read<I8> const& indset, LO key,
    LO* stack, Int stack_max) {
  if (!indset[key]) return 0;
  Int n = 0;
...
Graph get_buffered_elems(Mesh* mesh, Int key_dim, Read<I8> unbuffered_indset) {
  ...
  auto count = LAMBDA(LO key) {
    ...
    degrees_w[key] = find_distance_2_elems(keys2elems, elems2verts,
        nverts_per_elem, verts2elems, unbuffered_indset, key, stack, stack_max);
  };
  ...
  auto fill = LAMBDA(LO key) {
    ...
    find_distance_2_elems(keys2elems, elems2verts, nverts_per_elem, verts2elems,
...
Graph get_buffered_conflicts(
  ...
  auto fill = LAMBDA(LO key) {
    ...
    auto stack_max = end - begin;
    auto stack = &edges[begin];
    find_distance_3_keys(keys2buf_elems, elems2verts, nverts_per_elem,
        verts2elems, elems2keys, nkeys_per_elem, unbuffered_indset, key, stack,
        stack_max);
  };
  parallel_for(nkeys, fill);
  return Graph(offsets, edges);
