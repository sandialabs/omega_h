...
int main(int argc, char** argv) {
  ...
  mesh.balance(true);
  if (!mesh.comm()->rank()) std::cout << "imbalance after predictive " << mesh.imbalance() << '\n';
  if (vtk_path) writer.write();
  auto opts = Omega_h::AdaptOpts(&mesh);
  opts.verbosity = Omega_h::EXTRA_STATS;
  Omega_h::adapt(&mesh, opts);
  if (!mesh.comm()->rank()) std::cout << "imbalance after adapt " << mesh.imbalance() << '\n';
  if (vtk_path) writer.write();
  mesh.balance();
  if (!mesh.comm()->rank()) std::cout << "imbalance after post-balance " << mesh.imbalance() << '\n';
  if (vtk_path) writer.write();
