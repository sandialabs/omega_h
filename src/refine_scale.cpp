...
int main(int argc, char** argv) {
  ...
  if (world->rank() == 0)
    std::cout << "building a box of " << (6 * 7 * 7 * 210) << " elements!\n";
  auto mesh =
      Omega_h::build_box(world, OMEGA_H_SIMPLEX, 1e-6, 1e-6, 1e-3, 7, 7, 210);
  if (world->rank() == 0) std::cout << "done building the box, re-balancing!\n";
  mesh.balance();
  if (world->rank() == 0)
    std::cout << "done rebalancing, rank 0 has " << mesh.nelems()
              << " elements!\n";
  if (world->rank() == 0) std::cout << "ghosting!\n";
  mesh.set_parting(OMEGA_H_GHOSTED);
  if (world->rank() == 0)
    std::cout << "done ghosting, rank 0 has " << mesh.nelems()
              << " elements!\n";
  ...
  if (world->rank() == 0)
    std::cout << "done with implied metric, scaling it to " << ntarget_elems
              << " elements!\n";
  auto metrics = mesh.get_array<double>(0, "metric");
  auto scalar = Omega_h::get_metric_scalar_for_nelems(
      mesh.dim(), ncurrent_elems, ntarget_elems);
