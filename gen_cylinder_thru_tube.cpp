...
int main()
{
  ...
  auto cyl_disk = new_disk(
      gmod::Vector{0.5,0.5,0.1},
      gmod::Vector{0,0,1},
      gmod::Vector{1.0/8.0,0,0});
  auto cylinder = gmod::extrude_face(
      cyl_disk, gmod::Vector{0,0,0.5}).middle;
  gmod::add_to_group(g, cylinder);
  gmod::insert_into(fluid, cylinder);
  auto tube_disk = new_disk(
      gmod::Vector{0.5,0.5,0.7},
      gmod::Vector{0,0,1},
      gmod::Vector{1.0/8.0 + 2.0/32.0,0,0});
  auto tube_inner = new_circle(
      gmod::Vector{0.5,0.5,0.7},
      gmod::Vector{0,0,1},
      gmod::Vector{1.0/8.0 + 1.0/32.0,0,0});
  gmod::add_use(tube_disk, gmod::REVERSE, tube_inner);
  auto tube = extrude_face(
      tube_disk, gmod::Vector{0,0,0.5}).middle;
