...
void combine_pairs_and_cuts(
  ...
  CHECK(nkeys == keys2pairs_last.size() - 1);
  auto keys2ncuts = get_degrees(keys2cuts);
  auto keys2npairs_last = get_degrees(keys2pairs_last);
  auto keys2nprods = add_each(keys2ncuts, keys2npairs_last);
  keys2prods = offset_scan(keys2nprods);
  auto nprods = keys2prods.last();
  auto nppv = simplex_degrees[ent_dim][VERT];
  auto prod_verts2verts_w(nprods * nppv);
    ...
    for (auto pair = keys2pairs[key]; pair < keys2pairs[key]; ++pair) {
      for (Int ppv = 0; ppv < nppv; ++ppv) {
        prod_verts2verts_w[prod * nppv + ppv] =
          pair_verts2verts_last[pair * nppv + ppv];
      }
      ++prod;
    }
